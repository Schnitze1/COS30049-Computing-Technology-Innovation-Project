...
def plot_confusion_matrices(results: Dict[str, Dict[str, float]], out_dir: str = 'evaluation_reports', 
...
def plot_multiclass_metrics(results: Dict[str, Dict[str, float]], out_dir: str = 'evaluation_reports') -> str:
...
def plot_per_class_metrics(results: Dict[str, Dict[str, float]], traffic_types: List[str], 
                          out_dir: str = 'evaluation_reports') -> List[str]:
...
def save_label_metrics(label_metrics: Dict[str, Dict[str, float]], out_dir: str = 'evaluation_reports') -> str:
...
def export_multiclass_all(results: Dict[str, Dict[str, float]], traffic_types: List[str], 
	...
	# Save summary CSV
	paths['Multiclass Summary CSV'] = save_results_csv(results, out_dir, 'multiclass_metrics_summary.csv')
	
	# Save label metrics if provided
	if label_metrics:
		paths['Label-from-type CSV'] = save_label_metrics(label_metrics, out_dir)
	
	# Create visualizations
	try:
		paths['Multiclass Metrics Comparison'] = plot_multiclass_metrics(results, out_dir)
	except Exception as e:
		print(f"Warning: Could not create multiclass metrics plot: {e}")
	
	try:
		paths['Confusion Matrices'] = plot_confusion_matrices(results, out_dir, traffic_types)
	except Exception as e:
		print(f"Warning: Could not create confusion matrices: {e}")
	
	try:
		per_class_paths = plot_per_class_metrics(results, traffic_types, out_dir)
